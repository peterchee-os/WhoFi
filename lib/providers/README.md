...
Set `WHOFI_VERIFICATION_CLIENT_MAC` and optional `WHOFI_VERIFICATION_CLIENT_LABEL` when an operator wants a known client as a smoke-test anchor. The API only reports whether that anchor is configured and present; it does not echo the configured MAC address.
