...
- The dashboard Usage view renders current snapshot totals, rollups, source-filtered recent snapshot history, stored capture detail, selected-capture export, stored-capture replay, review notes, reviewed state, single-capture deletion, previous-capture deltas, new/missing device movement, and generated review-signal hints.
