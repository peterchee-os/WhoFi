...
- The dashboard Usage view renders current snapshot totals, rollups, source-filtered recent snapshot history, snapshot trend metrics and Markdown trend reports, tunable review policy thresholds, an open capture review queue with workload counts, source/severity queue filters, filtered queue Markdown report export, bulk visible-review closeout, stored capture detail, selected-capture JSON/Markdown export, stored-capture replay, review notes, reviewed state, single-capture deletion, previous-capture or explicit same-source baseline deltas, new/missing device movement, and generated review-signal hints.
