...
- Profiles device owner binding CSV/TSV import/export for local review handoff and demo recovery
...
- imported CSV profiles and ignored profile suggestions persist in the same browser-local demo review state and feed Profiles, owner mix, resolution, search, suggestion evidence, and assignment dropdowns; clearing imported profiles also clears owner overrides pointing at CSV-only profiles
...
523de72 Import CSV rosters into local profiles
9fd3033 Preview snapshot archive imports
6ffdf89 Compare snapshots against chosen baselines
a356753 Configure snapshot retention limits
939c6dc Prune snapshot history on demand
```
