...
- `GET /api/snapshot-history` returns recent local file-backed snapshot audit entries; `GET /api/snapshot-history/review-queue` returns open capture reviews plus workload counts; `GET /api/snapshot-history/review-queue/report` returns a filtered Markdown queue report; `PATCH /api/snapshot-history/review-queue` bulk-updates reviewed state for selected queue items; `GET /api/snapshot-history?include=captures` includes recent full captures; `GET /api/snapshot-history/{id}` returns one capture plus its previous same-source comparison, movement, and generated review-signal hints; `GET /api/snapshot-history/{id}/report` returns a Markdown capture report; `PATCH /api/snapshot-history/{id}` saves review notes and reviewed state; `DELETE /api/snapshot-history/{id}` removes one stored capture; `DELETE /api/snapshot-history` clears all captures.
