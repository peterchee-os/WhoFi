...
- `WHOFI_VERIFICATION_CLIENT_MAC` can configure one known device as a live-source smoke-test anchor; API responses report only whether the anchor is present, never the configured MAC value.
