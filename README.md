...
Imported and demo profiles also feed local profile-link suggestions. When an unclaimed device hostname appears to match a known person, team, or organization, the ledger and inspector surface the suggested owner, the inspector offers an `Assign Suggested` action, and the Dashboard/Devices views show a Profile Suggestions queue with source filters and `Assign Visible` bulk review. These suggestions are evidence for operator review, not automatic ownership claims.
...
Snapshot review policy is stored separately at `.whofi/snapshot-review-policy.json`. Set `WHOFI_SNAPSHOT_HISTORY_PATH` and `WHOFI_SNAPSHOT_REVIEW_POLICY_PATH` to move those files; set `WHOFI_SNAPSHOT_CAPTURE_LIMIT` and `WHOFI_SNAPSHOT_HISTORY_LIMIT` to tune the bounded local retention limits. `POST /api/snapshot-history/capture?source=demo|omada|omada-pp` captures and stores a snapshot, `GET /api/snapshot-history` returns summaries plus retention limits, `PATCH /api/snapshot-history` with `action: "prune"` applies current retention limits immediately, `GET /api/snapshot-history/export?source=all|demo|omada|omada-pp` returns a filtered JSON archive with entries, captures, review policy, review queue, and trends, `POST /api/snapshot-history/import?dryRun=true` validates an archive and returns importable/skipped/duplicate counts without writing, `POST /api/snapshot-history/import` merges a confirmed JSON archive into the bounded local audit store, `GET /api/snapshot-history/trends?source=all|demo|omada|omada-pp` returns timeline metrics, `GET /api/snapshot-history/trends/report?source=all|demo|omada|omada-pp` returns a filtered Markdown trend report, `GET/PATCH/DELETE /api/snapshot-history/review-policy` loads, saves, and resets queue thresholds, `GET /api/snapshot-history/review-queue` returns open capture reviews plus workload counts, `GET /api/snapshot-history/review-queue/report?source=all|demo|omada|omada-pp&severity=all|warning|watch` returns a filtered Markdown queue report, `PATCH /api/snapshot-history/review-queue` bulk-updates reviewed state for selected queue items, `GET /api/snapshot-history?include=captures` includes recent full captures, `GET /api/snapshot-history/{id}?compareTo={baseline_id}` returns one capture plus its delta from either the previous same-source capture or an explicit same-source baseline, `GET /api/snapshot-history/{id}/report` returns a Markdown audit report, `PATCH /api/snapshot-history/{id}` saves a review note and reviewed state, and `DELETE /api/snapshot-history/{id}` removes one stored capture. The Usage tab can filter history by source, show local retention counts, prune local history to current limits, show snapshot trend metrics, export/import source-filtered JSON archives with a required dry-run preview and confirm step, tune/export/import/reset review policy thresholds, filter the review queue by source/severity, bulk-mark visible queue items reviewed, inspect/export/delete/annotate stored captures, choose same-source comparison baselines, review open captures, download Markdown trend, queue, and capture reports, replay a stored capture into the current dashboard, and show saved totals, rollups, top devices, comparison deltas, new/missing device movement, and generated review-signal hints. This is a development/demo bridge toward database-backed, tenant-scoped observation and session persistence; `.whofi/` is ignored by git.
