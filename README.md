...
Snapshot review policy is stored separately at `.whofi/snapshot-review-policy.json`. Set `WHOFI_SNAPSHOT_HISTORY_PATH` and `WHOFI_SNAPSHOT_REVIEW_POLICY_PATH` to move those files. `POST /api/snapshot-history/capture?source=demo|omada|omada-pp` captures and stores a snapshot, `GET /api/snapshot-history` returns summaries, `GET /api/snapshot-history/export?source=all|demo|omada|omada-pp` returns a filtered JSON archive with entries, captures, review policy, review queue, and trends, `POST /api/snapshot-history/import` merges a JSON archive into the bounded local audit store, `GET /api/snapshot-history/trends?source=all|demo|omada|omada-pp` returns timeline metrics, `GET /api/snapshot-history/trends/report?source=all|demo|omada|omada-pp` returns a filtered Markdown trend report, `GET/PATCH/DELETE /api/snapshot-history/review-policy` loads, saves, and resets queue thresholds, `GET /api/snapshot-history/review-queue` returns open capture reviews plus workload counts, `GET /api/snapshot-history/review-queue/report?source=all|demo|omada|omada-pp&severity=all|warning|watch` returns a filtered Markdown queue report, `PATCH /api/snapshot-history/review-queue` bulk-updates reviewed state for selected queue items, `GET /api/snapshot-history?include=captures` includes recent full captures, `GET /api/snapshot-history/{id}` returns one capture plus its delta from the previous same-source capture, `GET /api/snapshot-history/{id}/report` returns a Markdown audit report, `PATCH /api/snapshot-history/{id}` saves a review note and reviewed state, and `DELETE /api/snapshot-history/{id}` removes one stored capture. The Usage tab can filter history by source, show snapshot trend metrics, export/import source-filtered JSON archives, tune/export/import/reset review policy thresholds, filter the review queue by source/severity, bulk-mark visible queue items reviewed, inspect/export/delete/annotate stored captures, review open captures, download Markdown trend, queue, and capture reports, replay a stored capture into the current dashboard, and show saved totals, rollups, top devices, comparison deltas, new/missing device movement, and generated review-signal hints. This is a development/demo bridge toward database-backed, tenant-scoped observation and session persistence; `.whofi/` is ignored by git.
